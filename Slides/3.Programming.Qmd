...
Here's how **`apply`** family functions can be used to compactly compute the *standard error of the mean* via *Monte Carlo simulation*

<div class="large-code">
```{r}
myVectorsList = lapply(1:10000, function(i) rnorm(30))

allMeans = sapply(myVectorsList, mean)

sd(allMeans)
```
</div>
<div style="font-size:22px; font-style:italic;"> 
1) `lapply` is used to populate a long list of vectors each including 30 randomly generated numbers; 
2) <b>`sapply` is then used to compute the `mean` of each vector in the list; </b>
3) *standard error of the mean* is computed as the `sd` on the vector of the *means*
</div>

<div style="font-size:24px; font-style:italic;"> Note: `"i"` represents the current element of `1:10000` over which `lapply` iterates. Even though it is practically useless here, because only random numbers are generated at each iteration, it must be included because `lapply` must pass an element as the argument to the function by default</div>
...
myFunctName = function(arg1 = NA, arg2 = NA, arg3 = TRUE){ 
  ...
  # body, series of operations
    ...
    res = ... get an object named "res" as final result 
