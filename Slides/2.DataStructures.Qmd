...
Know the **length** of a vector using the `length()` function:
...
Extract elements with **INDEXING** using square brackets `[]`:

```{r}
Hours[4] # a single element
Hours[5:7] # a range of elements
Hours[c(1,3,6)] # specific elements
Hours[length(Hours)] # the last element
```
</div>
...
here is how I would import it in R, and display the first few rows:
